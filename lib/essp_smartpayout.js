...
export default class eSSP extends EventEmitter {
  constructor () {
    ...
    this.valueMultiplier = 100; // updated from SETUP_REQUEST
  }
    ...
    parser.on ('data', buffer => {
        ...
        if (len2 - len != 0) {
          ...
          for (let i = 0; i < n_7f7f; i++) {
            ...
            while (pos7f7f !== -1 && pos7f7f % 2 !== 0) {
              // '7f7f' matched across a byte boundary -- keep searching
              pos7f7f = bufferstr.indexOf ('7f7f', pos7f7f + 1);
            }
            if (pos7f7f === -1) break;
            bufstr =
  ...
  */
  ...
  async drain () {
    // serialport v13 drain() is callback-based and returns undefined,
    // so it has to be wrapped to be awaitable
  ...
  async retransmit_lastBuff () {
    // resend the exact same bytes: same sequence flag, same encryption
    // counter, byte stuffing already applied by toPackets()
    let buff = Buffer.from (this.last_packet);
...
*/
   ...
   */
    ...
    // SSP byte stuffing: any 0x7f in the packet body (after the STX)
    // must be duplicated, otherwise the device treats it as a new STX
    let stuffed = [commandLine[0]];
    for (let i = 1; i < commandLine.length; i++) {
      stuffed.push (commandLine[i]);
      if (commandLine[i] === 0x7f) {
        stuffed.push (0x7f);
      }
    }
    commandLine = stuffed;
      ...
      if (buf[3] == 0x7e) {
        ...
        if ((buf[2] - 1) % 16 == 0) {
          ...
          if (this.response_count < this.count) {
            this.count = this.response_count;
            this.flagRetransmit = 1;
            console.log ('edata_page=', edata_page);
      ...
      if (this.currentCommand == 'REQUEST_KEY_EXCHANGE') {
        ...
        if (data[0] == 240) {
          ...
          while (i < nominal) {
            ...
            // record: 2 bytes level, 4 bytes value, 3 bytes currency
            level[0] = parseInt (
              convertHex.bytesToHex (nom.slice (0, 2).reverse ()),
              16
            );
            level[1] =
              parseInt (
                convertHex.bytesToHex (nom.slice (2, 6).reverse ()),
                16
        ...
        if (data[0] == 240) {
          ...
          let valueMultiplier = parseInt (
            this.hexByte (data[13 + channelsCount * 2]) +
              this.hexByte (data[13 + 1 + channelsCount * 2]) +
              this.hexByte (data[13 + 2 + channelsCount * 2]),
            16
          );
          this.valueMultiplier = valueMultiplier || 100;
          ...
          for (let i = 0; i < channelsCount * 1; i++) {
            let denomination = parseInt (
              this.hexByte (data[17 + 3 + channelsCount * 5 + i * 4]) +
                this.hexByte (data[17 + 2 + channelsCount * 5 + i * 4]) +
                this.hexByte (data[17 + 1 + channelsCount * 5 + i * 4]) +
                this.hexByte (data[17 + 0 + channelsCount * 5 + i * 4]),
              16
            );
    ...
    if (error.code !== 0xf0) {
      ...
      switch (data[1]) {
        ...
        case 0xd2: //SMART payout|SMART Hopper|nv11
          ...
          break;
        case 0xd5: //SMART payout|SMART Hopper|nv11
          event = ['jammed', data[2]];
          break;
